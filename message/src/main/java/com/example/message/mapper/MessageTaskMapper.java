...
//    **************************五大基础方法**************************
	...
	@Update("update message_task set status = 100, update_time = #{timestamp} where id = #{id} and status = 0 and is_deleted = 0 limit 1")
	int opLock(@Param("id") Long id, @Param("timestamp") Integer timestamp);
}
